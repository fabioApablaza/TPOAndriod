...
public class ContactosCumpleanio extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_contactos_cumpleanio);
    ...
    private void listar() {
        ...
        if(cursor!=null && cursor.getCount() > 0)
        {
            if (cursor.moveToFirst())
            {
                do {
                    cont = new Contacto();
                    cont.setNombre(cursor.getString(0));
                    cont.setApellido(cursor.getString(1));
                    cont.setTelefono(cursor.getString(2));
                    cont.setDireccionPostal(cursor.getString(3));
                    cont.setFechaNac(cursor.getString(4));
